...
const inputAnalysis = readFileSync("src/lib/prompt/input-analysis.ts", "utf8");
...
assertMatches(
  /if \(draft\.source\.startsWith\("library-"\)\) \{[\s\S]*?new URLSearchParams\(\{[\s\S]*?studioSource: draft\.source[\s\S]*?\}\)[\s\S]*?draft\.sourceVariant[\s\S]*?params\.set\("studioVariant", draft\.sourceVariant\)[\s\S]*?normalizeInternalHref\(`\/library\?\$\{query\}`\) \?\? "\/library"[\s\S]*?\}[\s\S]*?if \(draft\.source\.startsWith\("skills-"\)\) \{[\s\S]*?return "\/skills";[\s\S]*?\}[\s\S]*?if \(draft\.source\.startsWith\("profile-"\)\) \{[\s\S]*?return "\/profile";[\s\S]*?\}[\s\S]*?if \(draft\.source\.startsWith\("company-"\)\) \{[\s\S]*?return "\/company";[\s\S]*?\}[\s\S]*?return "\/learning";/,
