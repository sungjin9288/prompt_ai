...
assertFileIncludesInOrder(
  ...
  "Supabase RLS smoke test checklist should keep workspace/owner target identity, role fixtures, owner/member/viewer/non-member behavior, cross-workspace isolation, evidence capture, and acceptance gates together",
);
...
assertFileIncludesInOrder(
  ...
  "Data post-import evidence should keep identity, required evidence, acceptance gates, expected insert order, evidence slots, and rollback triggers together",
);
...
assertFileIncludesInOrder(
  ...
  "Supabase import route should accept backupJson or backup payloads and validate app, schema, and required backup data before import planning",
);
assertFileIncludesInOrder(
  supabaseImportRouteSource,
  [
...
assert.match(
  supabaseImportRouteSource,
  /if \(execute\) \{[\s\S]*?if \(!environmentStatus\.executionEnabled\) \{[\s\S]*?createSupabaseImportBlockedResponse\(\{[\s\S]*?httpStatus: 403[\s\S]*?status: "execution-disabled"[\s\S]*?if \(body\.confirmation !== SUPABASE_IMPORT_CONFIRMATION\) \{[\s\S]*?createSupabaseImportBlockedResponse\(\{[\s\S]*?httpStatus: 400[\s\S]*?status: "confirmation-required"[\s\S]*?if \([\s\S]*?!environmentStatus\.supabaseUrlConfigured[\s\S]*?!environmentStatus\.serviceRoleKeyConfigured[\s\S]*?\) \{[\s\S]*?createSupabaseImportBlockedResponse\(\{[\s\S]*?httpStatus: 503[\s\S]*?status: "environment-incomplete"[\s\S]*?if \(!validation\.ok\) \{[\s\S]*?createSupabaseImportBlockedResponse\(\{[\s\S]*?httpStatus: 422[\s\S]*?status: "validation-blocked"[\s\S]*?createSupabaseRestImportAdapter[\s\S]*?runSupabaseImportExecutionPlan/,
...
assertFileIncludesInOrder(
  ...
  "Supabase import blocked response helper should keep the full operator payload and HTTP status together",
);
...
assertFileIncludesInOrder(
  ...
  [
    ...
    "            (tableResult) =>",
    "`- ${tableResult.order}. ${tableResult.table}: ${tableResult.status} / inserted ${tableResult.insertedRows}/${tableResult.expectedRows}",
  ],
