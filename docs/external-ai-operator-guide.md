...
3. 증거 저장: `npm run smoke:integrations`로 Chrome, MCP bridge, MCP client, Learning smoke evidence file을 먼저 남깁니다.
...
client 호출 흐름까지 확인하려면 stdio JSON-RPC client smoke를 실행합니다.
