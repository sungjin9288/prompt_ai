...
- Integrations Smoke 증거 경로는 local packet, actual evidence, reviewed feedback record 순서를 먼저 보여주고 MCP bridge, Chrome popup, Learning feedback 큐 smoke가 각각 command, evidence, result로 어떻게 이어지는지 대조하며 상세 섹션으로 이동하게 해야 한다.
...
- Integrations 운영자 다음 조치는 로컬 서버 유지, 연결 표면 1개 검증, 로컬 smoke evidence 저장, 외부 AI 전달 전 검토, 실행 결과 피드백 저장, release evidence와 release-candidate 확인을 같은 실행 순서로 보여주고 Chrome loaded extension, MCP client, Learning feedback 실제 증빙 필드를 먼저 확인하게 해야 한다.
