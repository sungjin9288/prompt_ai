...
assert.match(
  readme,
  /checks that the import route returns explicit\s+execution-disabled, confirmation-required, environment-incomplete, and\s+validation-blocked responses with status codes before any write adapter can run/,
...
assert.match(
  readme,
  /checks that the verification report keeps import target, execution\s+order, replacement guidance, acceptance checks, warnings, rollback triggers,\s+and sign-off together/,
  "README should document the data-management Supabase verification report package verification scope",
);
assert.match(
  readme,
  /checks that the migration rehearsal report keeps preflight identity,\s+readiness checklist, acceptance gates, insert order, blockers, and required\s+artifacts together/,
  "README should document the data-management Supabase migration rehearsal report verification scope",
);
assert.match(
  readme,
...
assert.match(
  readme,
  /checks that the controlled execution packet keeps the packet index,\s+non-execution guard, secret guard, and every required execution artifact in\s+order/,
  "README should document the data-management Supabase controlled execution packet verification scope",
);
assert.match(
  readme,
  /checks that the execution packet manifest keeps readiness items and\s+copy actions in the operator review order/,
  "README should document the data-management Supabase execution packet manifest verification scope",
);
assert.match(
  readme,
  /checks that the execution packet next-action memo keeps identity,\s+manifest summary, waiting detail, copy actions, next action, and the\s+non-execution guardrail in memo order/,
  "README should document the data-management Supabase execution packet next-action memo verification scope",
);
assert.match(
  readme,
  /checks that the API preflight report keeps status, validation,\s+dry-run metrics, UUID metrics, blockers, insert order, and no-write guard\s+together/,
  "README should document the data-management Supabase API preflight report verification scope",
);
assert.match(
  readme,
