...
- Integrations 운영 흐름 요약에서 Capture, Refine, Evidence, Deliver, Feedback 단계별 owner, artifact, gate를 카드로 먼저 보여준 뒤 상세 표를 유지합니다. 섹션별 용도 설명이 포함된 빠른 이동 내비게이션과 Refine API 테스트, MCP 연결 설정, 준비도, 환경별 실행 가이드, Feedback inbox 상세 섹션 앵커 이동 제공
...
- `extensions/chrome` in-page 개선: `chatgpt.com`/`chat.openai.com`/`claude.ai`/`gemini.google.com` composer 옆에 MV3 content script로 `개선` 버튼을 띄우고, 클릭 시 초안을 background service worker 경유로 `POST /api/integrations/refine`에 보내 개선된 프롬프트를 입력창에 삽입합니다. 결과 바에서 `되돌리기`/`복사`/`Studio에서 열기`를 제공하고, 사이트 DOM 변화에 대비해 adapter별 fallback selector 체인과 debounced MutationObserver로 재부착하며 selector가 안 맞으면 버튼 없이 조용히 종료해 host 페이지를 깨뜨리지 않습니다. `Studio에서 열기`는 개선된 프롬프트를 `/improve?source=extension&origin=<adapterId>&draft=<encoded>`로 전달해 `/improve` 초점형 페이지를 새 탭에서 엽니다.
- `/improve` 초점형 개선 페이지: 확장 프로그램 또는 직접 방문으로 전달된 초안 하나를 로컬 엔진으로 개선하는 단일 화면입니다. `draft`/`source`/`origin` 쿼리 파라미터를 파싱(최대 4000자로 절단)하고, `source=extension`이면 전달 출처 안내 문구를 보여줍니다. `개선하기`는 네트워크 호출 없이 로컬 `createPromptPackage`를 직접 호출해 품질 점수와 함께 결과를 보여주고, `프롬프트 복사`/`복사 후 {AI}에서 열기`/`라이브러리에 저장`/`Studio에서 계속` 액션을 제공합니다. Studio 전달이 실패하면 이동하지 않고 수동 복사용 원문을 표시합니다.
...
- 전역 커맨드 팔레트: `⌘K`/`Ctrl+K` 또는 헤더 검색 버튼으로 열어 저장본, 스킬, 학습 메모리를 검색하고 모든 화면으로 바로 이동. 접근성 dialog(combobox/listbox, 포커스 관리, 결과 수 안내)로 구성하고 고정 저장본을 먼저 보여줌
- 전역 키보드 단축키: `?`로 단축키 도움말 오버레이, `g` 프리픽스로 화면 이동(`g h`, `g l`, `g m` 등), `Esc`로 닫기. 입력 필드에 포커스가 있으면 무시하며 도움말은 접근성 dialog로 표시
- 최근 활동 타임라인(Dashboard `/#activity` 섹션): 프롬프트 생성·개선, 스킬 실행, 피드백, 스킬·메모리 추가 이력을 최신순으로 날짜별 그룹으로 확인하고, 이벤트 유형별 필터와 유형별 개수 요약 제공. 예전 `/activity` 경로는 `/#activity`로 리다이렉트된다.
- Library 저장본 태그: 자유 태그로 저장본을 정리하고 태그 필터(`?tag=` URL 동기화)와 커맨드 팔레트 검색에 반영
- Library 저장본 즐겨찾기/핀: 자주 쓰는 저장본을 고정해 목록 상단에 먼저 보여주고 `고정됨` 배지 표시, 커맨드 팔레트에서도 먼저 노출
- Library 저장본 내보내기: 선택 저장본을 Markdown 또는 재가져오기 가능한 JSON 파일로 다운로드
- Library 저장본 복제: 선택 저장본을 `(사본)`으로 복제하되 내용·버전·태그는 유지하고 피드백·핀·출처 이력은 초기화한 독립 사본 생성
