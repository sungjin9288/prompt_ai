...
개인 맥락과 회사 맥락을 한 화면의 섹션 전환(`개인 맥락`/`회사 맥락`)으로 관리하는 통합 기준 화면. `?section=` 쿼리 또는 `#profile`/`#company` 해시로 특정 섹션에 바로 진입할 수 있으며, 예전 `/profile`, `/company` 경로는 각각 `/context#profile`, `/context#company`로 리다이렉트된다.
...
- Integrations operations checklist 계열 Studio 복귀 액션 라벨은 `Integrations 원본 섹션으로 돌아가기`로 표시해 환경별 실행 가이드, MCP smoke runbook 초안이 저장한 원본 섹션 앵커로 돌아가게 한다
