...
- Integrations 운영자 다음 조치는 로컬 서버 유지, 연결 표면 1개 검증, 로컬 smoke evidence 저장, 외부 AI 전달 전 검토, 실행 결과 피드백 저장, release evidence와 release-candidate 확인을 같은 실행 순서로 보여줘야 한다.
