...
제출을 시작하기 전에 아래를 모두 확인하세요. 하나라도 완료되지 않았다면
제출을 시작하지 마세요 (특히 `{PRODUCTION_URL}` 채움은 개인정보처리방침
URL·웹사이트 URL 필드에 필수입니다).

- [ ] `docs/launch-plan.md`의 운영자 게이트 체크리스트 중 "Vercel 계정 연결·
      프로젝트 생성"이 완료되어 프로덕션 URL이 확정되었다.
- [ ] `src/lib/site/config.ts`의 `siteUrl`이 배포 환경에서 `NEXT_PUBLIC_APP_URL`
      환경 변수로 실제 프로덕션 도메인을 가리키도록 Vercel에 설정되었다.
- [ ] 프로덕션 배포에서 `{PRODUCTION_URL}/privacy`가 정상적으로 열린다.
- [ ] 프로덕션 배포에서 `{PRODUCTION_URL}/welcome`이 정상적으로 열린다.
- [ ] `extensions/chrome/manifest.json`의 `version`이 이번에 제출할 최종
      버전이다 (이름을 바꾸지 않는 한 `0.4.0` 유지, 매니페스트를 다시 손대면
...
1. "새 항목(New item)"에서 `output/extension/prompt-ai-studio-refine-v0.4.0.zip`
...
8. **개인정보처리방침 URL** — `{PRODUCTION_URL}/privacy`를 실제 도메인으로
   치환해 입력.
9. **웹사이트** — `{PRODUCTION_URL}/welcome`을 실제 도메인으로 치환해 입력.
...
`{PRODUCTION_URL}/privacy`와 `listing.ko.md`/`listing.en.md`의 데이터 사용
공개 매핑 표).
...
3. 변경을 커밋하고 프로덕션을 재배포합니다 (`npm run build` 확인 후 Vercel
   재배포, 또는 프로젝트의 표준 배포 절차를 따릅니다).
4. `docs/launch-plan.md`의 P52 항목이 이미 "완료"로 표시되어 있는지 확인하고,
   실제 스토어 URL이 반영된 배포가 라이브인지 `{PRODUCTION_URL}`에서
   `/welcome` 등 확장 링크가 걸린 화면을 열어 최종 확인합니다.
