...
- Integrations 운영 흐름 요약에서 Capture, Refine, Evidence, Deliver, Feedback 단계별 owner, artifact, gate를 카드로 먼저 보여준 뒤 상세 표를 유지합니다. 섹션별 용도 설명이 포함된 빠른 이동 내비게이션과 Refine API 테스트, MCP 연결 설정, Smoke 증거 경로, 준비도, 환경별 실행 가이드, Feedback inbox 상세 섹션 앵커 이동 제공
...
- 최근 활동 타임라인(`/activity`): 프롬프트 생성·개선, 스킬 실행, 피드백, 스킬·메모리 추가 이력을 최신순으로 날짜별 그룹으로 확인하고, 이벤트 유형별 필터와 유형별 개수 요약 제공
