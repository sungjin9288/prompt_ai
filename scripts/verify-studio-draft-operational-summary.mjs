...
assertMatches(
  /function buildLibraryStudioSourceHref\([\s\S]*?new URLSearchParams\(\{ studioSource: source \}\)[\s\S]*?studioPersistence[\s\S]*?params\.set\("studio", studioPersistence\)[\s\S]*?promptId[\s\S]*?params\.set\("prompt", promptId\)[\s\S]*?sourceVariant[\s\S]*?params\.set\("studioVariant", sourceVariant\)[\s\S]*?return buildLibraryHref\(params\)[\s\S]*?if \(draft\.source\.startsWith\("library-"\)\) \{[\s\S]*?return buildLibraryStudioSourceHref\(\{[\s\S]*?source: draft\.source[\s\S]*?sourceVariant: draft\.sourceVariant[\s\S]*?\}\)[\s\S]*?\}[\s\S]*?if \(draft\.source\.startsWith\("skills-"\)\) \{[\s\S]*?return "\/skills";[\s\S]*?\}[\s\S]*?if \(draft\.source\.startsWith\("profile-"\)\) \{[\s\S]*?return "\/context\?section=profile#profile";[\s\S]*?\}[\s\S]*?if \(draft\.source\.startsWith\("company-"\)\) \{[\s\S]*?return "\/context\?section=company#company";[\s\S]*?\}[\s\S]*?return "\/learning";/,
